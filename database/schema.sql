...
-- Phiên bản: 5.1 - Final Consolidated (Hợp nhất Schema & Migrations)
...
CREATE TABLE `ky_hop` (
  ...
  `deadline_dang_ky` date DEFAULT NULL,
  `deadline_phe_duyet` date DEFAULT NULL,
  `deadline_hoan_thien` date DEFAULT NULL,
...
CREATE TABLE `noi_dung` (
  ...
  `nguoi_phe_duyet_id` int DEFAULT NULL COMMENT 'Lãnh đạo VP được chọn',
  `phong_ban_id` int DEFAULT NULL,
  `stt` int DEFAULT 999,
...
-- 9. BẢNG THÔNG BÁO (Đã tích hợp ngay_doc và Indexes v5.1)
...
CREATE TABLE `template_checklist` (
  ...
  `mo_ta` text,
  `danh_sach_cong_viec` text NOT NULL COMMENT 'JSON array',
  `trang_thai` tinyint(1) DEFAULT 1,
  `created_at` timestamp DEFAULT CURRENT_TIMESTAMP,
  `updated_at` timestamp DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
...
CREATE TABLE `danh_gia_can_bo` (
  ...
  `user_id` int NOT NULL,
  `ky_hop_id` int NOT NULL,
  `so_luong_noi_dung` int DEFAULT 0,
  `diem_dung_han` decimal(5,2) DEFAULT 0.00,
  `diem_chat_luong` decimal(5,2) DEFAULT 0.00,
  `so_lan_doc_viec` int DEFAULT 0,
...
-- DATA MẪU - ADMIN USER (Password: admin123)
